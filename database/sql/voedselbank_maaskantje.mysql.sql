...
ON DUPLICATE KEY UPDATE
    ...
    verantwoordelijkheden = new.verantwoordelijkheden,
    bio = new.bio,
    updated_at = NOW();
...
ON DUPLICATE KEY UPDATE
    adres = new.adres,
    contactpersoon_naam = new.contactpersoon_naam,
    contactpersoon_email = new.contactpersoon_email,
    telefoonnummer = new.telefoonnummer,
    volgende_levering = new.volgende_levering,
    updated_at = NOW();
...
WHERE NOT EXISTS (
    SELECT 1
    FROM klanten
    WHERE gezinsnaam = 'Familie Jansen'
      AND telefoonnummer = '06-11111111'
);
...
WHERE NOT EXISTS (
    SELECT 1
    FROM klanten
    WHERE gezinsnaam = 'Familie De Vries'
      AND telefoonnummer = '06-22222222'
);
...
WHERE NOT EXISTS (
    SELECT 1
    FROM klanten
    WHERE gezinsnaam = 'Familie Bakker'
      AND telefoonnummer = '06-33333333'
);
...
SELECT
    'Familie El Idrissi',
    'Waterweg 4, Maaskantje',
    '06-44444444',
    'elidrissi@voorbeeld.nl',
    'afwezig',
    'Vegetarisch',
    2,
    2,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM klanten
    WHERE gezinsnaam = 'Familie El Idrissi'
      AND telefoonnummer = '06-44444444'
);
...
SELECT
    'Familie Van Dijk',
    'Kerkstraat 2, Maaskantje',
    '06-55555555',
    'vandijk@voorbeeld.nl',
    'binnen_land',
    'Pinda''s',
    1,
    1,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM klanten
    WHERE gezinsnaam = 'Familie Van Dijk'
      AND telefoonnummer = '06-55555555'
);
...
AS new
ON DUPLICATE KEY UPDATE
    productnaam = new.productnaam,
    aantal_in_voorraad = new.aantal_in_voorraad,
    categorie_id = new.categorie_id,
    leverancier_id = new.leverancier_id,
    updated_at = NOW();
...
WHERE NOT EXISTS (
    SELECT 1
    FROM leverancier_products lp
    WHERE lp.leverancier_id = l.id
