...
BEGIN
    ...
    DECLARE v_bestaat_gezinsnaam INT DEFAULT 0;
    ...
    IF v_klant_bestaat = 0 THEN
        SELECT 0 AS gewijzigd, 0 AS bestaat_al, 0 AS klant_bestaat;
    ELSE
        SELECT COUNT(*)
        INTO v_bestaat_gezinsnaam
        FROM klanten
        WHERE id <> p_klant_id
            AND gezinsnaam = p_gezinsnaam;

        IF v_bestaat_gezinsnaam > 0 THEN
            SELECT 0 AS gewijzigd, 1 AS bestaat_al, 1 AS klant_bestaat;
        ELSE
            UPDATE klanten
            SET
            ...
            SELECT 1 AS gewijzigd, 0 AS bestaat_al, 1 AS klant_bestaat;
        END IF;
    END IF;
END
SQL);
